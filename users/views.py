...
from .auth import JWTAuthentication, generate_token
...
def register(request):
    ...
    if data['password'] != data['password_confirm']:
        raise exceptions.APIException('Password confirmation does not match.')
...
def login(request):
    ...
    if user is None:
        raise exceptions.AuthenticationFailed('User do not exists.')

    if not user.check_password(password):
        raise exceptions.AuthenticationFailed('Email or password do not match.')
    ...
    userSerialized = UserSerializer(user).data
